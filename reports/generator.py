...
def _enrich_report_data(report_data: Dict[str, Any]) -> Dict[str, Any]:
    """Helper to enrich report data with Plain-English danger analysis and code fix recipes."""
    findings = report_data.get("all_findings") or report_data.get("findings") or []
    target_url = str(report_data.get("target") or report_data.get("target_url") or "")

    posture = calculate_security_posture(findings)
    report_data["posture"] = posture

    for f in findings:
        f["danger"] = analyze_real_danger(f)
        f["recipes"] = get_fix_recipes(f)
        f["dev_ticket"] = generate_dev_ticket_markdown(f, target_url=target_url)

    return report_data


class ReportGenerator:
    """Generate HTML and PDF reports from scan results."""
    ...
    def generate_html(self, report: Report, output_path: Optional[str] = None) -> str:
        ...
        report_dict = _enrich_report_data(report_dict)
    ...
    def generate_pdf(self, report_data: Dict[str, Any], output_path: str) -> str:
        ...
        # Executive Posture & Real Danger Summary
        posture = report_data.get("posture") or calculate_security_posture(findings)
        grade_text = f"<b>SECURITY GRADE: <font color='{posture['grade_color']}' size='12'>{posture['grade']} ({posture['grade_label']})</font></b>"
        story.append(Paragraph(grade_text, h2_style))
        story.append(Paragraph(f"<b>Executive Summary:</b> {saxutils.escape(posture['summary'])}", sub_style))
        story.append(Spacer(1, 8))

        # Findings List
        story.append(Paragraph('Detailed Findings & Plain-English Danger Assessment', h2_style))
        ...
        for idx, f in enumerate(findings, 1):
            ...
            danger_info = f.get("danger") or analyze_real_danger(f)
            danger_headline = saxutils.escape(danger_info.get("headline", ""))
            danger_attacker = saxutils.escape(danger_info.get("what_attacker_can_do", ""))
            ...
            danger_p = Paragraph(f'<b>🚨 Real-World Danger:</b> {danger_headline} — {danger_attacker}', danger_style)
            desc_p = Paragraph(f'<b>Technical Details:</b> {desc_text}', finding_body)

            card_elements = [ht, Spacer(1, 3), meta_p, Spacer(1, 3), danger_p, Spacer(1, 3), desc_p]
